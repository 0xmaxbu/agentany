...
日期：2026-08-15（PRD v1 路线 grilling 定稿；同日 M4 实践层 grill 两轮修订：任务本质+权限模型；2026-08-16 决策 7 二次修订见 ADR-0023）

## 状态

已接受（2026-08-15 修订：任务=LLM 制定的自由 prompt 任务；member 自建自批；对话可改任务；2026-08-16 决策 7 修订：admin 经 UI/API 全管理 system 任务——详见 ADR-0023）
...
7. **管理三面**：admin 管理页（**全部任务含所有 member 的与 system**：查看/新建/修改（含权限开关）/停用/手动跑/删除——删除须无在跑 run，409 与手动跑同口径）；member 在 chat 右侧面板或对话中管理**自己建的**任务；chat 对话支持**改任务**（LLM 定位旧任务→改 cron/prompt→新任务卡确认）。**system 任务经 chat 工具只读且仅 admin 可读**——LLM 工具（bridge /task/*）对 system 行的建/删/停/改一律直接拒（不论角色；system 任务管理仅 admin UI/API 一条通道）（2026-08-16 两次修订：初版「admin UI 也只读」→ ADR-0023 决策 4 放开为 admin 经 UI/API 全管理；蒸馏 seed 例外冻结——仅 cron 可改、不可新建/删）。
