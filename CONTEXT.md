...
| 登录态 (Login session) | 设备浏览器持久 profile 中的**已登录会话**（cookies/localStorage）。**只存设备本地**：远端操作者/服务器拿不到 cookie 原文，只是"借用设备用户已授权会话"在设备用户名下做动作；借用须经设备侧[会话借用授权](#会话借用授权-session-borrow-consent)（ADR-0035/0038）。 |
| 工作流授权 (Workflow grant) | **远端用户**授权某工作流可使用 remote 执行（`workflow_grants` 表，服务器侧判定）。管"这个工作流能不能用 remote"。_Avoid:_ 与设备侧会话借用授权混称 |
| 会话借用授权 (Session-borrow consent) | **设备用户**对"某工作流借用本机登录会话/桌面控制"的本地授权：客户端分派层统一拦截点按规则引擎决定放行/弹窗/拒绝（默认仅借用类工具首次询问并记住；策略四档可按工作流×工具×站点粒度调整），存 `~/.agentany/grants.json`（ADR-0038）。管"能不能用**我的**会话"——与工作流授权构成双层。拒绝=工具级失败透传（run 不中止），卡死由设备放开+远端手动重跑恢复 |
