...
| 定时任务 (Scheduled Task) | 用户配置的 **cron 触发器**，**仅触发 LLM 可独立完成的任务**（**不触发工作流**——工作流含 HITL/审批语义，cron 无人值守不适用）。**任务本质=自由 prompt 任务**：用户在 chat 里说需求（如「每 4 小时去 xx 网站读新闻发摘要」），LLM 解析出 cron+任务 prompt → **任务卡确认**（cron 人类可读+未来 3 次执行时间+频率下限校验）→ 入库；到点=pi 以该 prompt 跑（同 chat 沙箱），产出投递**产出会话**。两类 scope：**workspace**（成员随口建，绑建时 ws）/ **system**（跨 ws 内置如经验蒸馏，代码 seed DB 行；**经 chat 删除/停用一律服务端硬拒**，仅 admin UI 可管）。**成员自建自批**（任务卡自己确认即建；CommandPolicy 仍生效——deny 拒、require_approval 发卡 admin 批）；对话/面板可改可管自己的任务，admin 管理页管全部。执行不再逐次问（创建即授权）；支持手动调用；错过窗口记 missed 不补跑；同任务在跑→跳过（skipped_overrun）。 |
| 产出会话 (Output conversation) | 定时任务创建时自动建的**专属会话**（挂任务同 workspace）：任务每次执行的产出投递于此——用户打开 chat 即见历史产出流。创建者=建任务的用户。 |
