...
describe("registry.start 门控（#18）", () => {
  test("allow（synthetic under auto）→ {running, runId} + run_started；直跑不经审批", async () => {
    ...
    const r = await registry.start({ conversationId: "c-appr", workflowId: "synthetic-3step", input: {} });
  ...
  test("require_approval（brand-research under auto）→ {needs_approval}；不建 run；建审批卡（kind=approval/runId=null/存 workflowId+input）；发 hitl_request{kind:approval}", async () => {
    const { store, eventBus, registry } = setup();
    const frames: any[] = [];
    eventBus.subscribe("c-appr", (f) => frames.push(f));
    const input = { brand: "测试品牌" };
    const r = await registry.start({ conversationId: "c-appr", workflowId: "brand-research", input });
  ...
  test("require_approval 幂等：同 conv+workflow 再 start → 同 questionId，不建新卡/不发新帧", async () => {
    const { store, eventBus, registry } = setup();
    const frames: any[] = [];
    eventBus.subscribe("c-appr", (f) => frames.push(f));
    const r1 = await registry.start({ conversationId: "c-appr", workflowId: "brand-research", input: { brand: "a" } });
    if (r1.status !== "needs_approval") throw new Error("expected needs_approval");
    const before = frames.length;
    const r2 = await registry.start({ conversationId: "c-appr", workflowId: "brand-research", input: { brand: "a" } });
  ...
  test("approved flag 跳 policy（/approvals decide 内部用）：brand-research + approved → 直跑 running", async () => {
    ...
    const frames: any[] = [];
    eventBus.subscribe("c-appr", (f) => frames.push(f));
    const r = await registry.start({ conversationId: "c-appr", workflowId: "brand-research", input: { brand: "x" }, approved: true });
    expect(r.status).toBe("running");
    if (r.status !== "running") throw new Error("expected running");
    await delayUntil(() => frames.some((f) => f.type === "run_started"));
...
describe("审批卡应答（统一卡应答 · 消息绑定，#28 重构）", () => {
  test("approve：{approve} → {approved, runId}；run_started；卡 answered + 回填 runId + decidedBy + hitl_answered{approval}", async () => {
    ...
    const frames: any[] = [];
    eventBus.subscribe("c-appr", (f) => frames.push(f));
    const r = await registry.start({ conversationId: "c-appr", workflowId: "brand-research", input: { brand: "测试品牌" } });
    if (r.status !== "needs_approval") throw new Error("expected needs_approval");
  ...
  test("deny：{deny} → {denied}；不 createRun（无 run_started）；卡 answered{deny} + runId 仍 null", async () => {
    ...
    const frames: any[] = [];
    eventBus.subscribe("c-appr", (f) => frames.push(f));
    const r = await registry.start({ conversationId: "c-appr", workflowId: "brand-research", input: { brand: "x" } });
    if (r.status !== "needs_approval") throw new Error("expected needs_approval");
  ...
  test("双击幂等：approve 后再 approve → 409 already decided（CAS）", async () => {
    ...
    const app = createApp(fullDeps(store, { runLifecycle: registry }));
    const r = await registry.start({ conversationId: "c-appr", workflowId: "brand-research", input: { brand: "x" } });
    if (r.status !== "needs_approval") throw new Error("expected needs_approval");
...
describe("审批只人类 enforce + HTTP 旁路（#18）", () => {
  ...
  test("HTTP 旁路锁定（ADR-0031 决策 2）：POST /workflows/brand-research/runs 走同一 gate → require_approval + 无会话锚 → 被堵（400 invalid input），不 createRun、无 approval 卡", async () => {
    const { store, eventBus, registry } = setup();
    const app = createApp(fullDeps(store, { runLifecycle: registry, eventBus, runPiFactory: stubFactory as any }));
    const frames: any[] = [];
    eventBus.subscribe("c-appr", (f) => frames.push(f));
    const resp = await app.request("/workflows/brand-research/runs", { method: "POST", headers: JH, body: JSON.stringify({ input: { brand: "x" } }) });
    expect(resp.status).toBe(400); // require_approval 工作流只能从会话起（审批卡需回复锚）；直调被堵
    expect(store.hitl.listQuestions("c-appr", { includeAnswered: true, kind: "approval" })).toHaveLength(0); // 无卡（谁也不该盲批）
...
describe("approve CAS 顺序 + 失败回滚（#codex review：占位不得永久卡死审批）", () => {
  test("registry 不可用 → 503 且卡仍 pending（可重试，不永久卡死）", async () => {
    ...
    const r = await registry.start({ conversationId: "c-appr", workflowId: "brand-research", input: { brand: "x" } });
    if (r.status !== "needs_approval") throw new Error("expected needs_approval");
    const app = createApp(fullDeps(store, { runLifecycle: undefined })); // 无 lifecycle → dispatch 报错但卡可重试
    const resp = await answerCard(app, r.questionId, "批准");
  ...
  test("start() 抛错 → 500 + 回滚卡为 pending（可重试，不永久卡死）", async () => {
    ...
    const app = createApp(fullDeps(store, { runLifecycle: boomRegistry as any }));
    const resp = await answerCard(app, r.questionId, "批准");
    expect(resp.status).toBe(202);
    await delay(20);
