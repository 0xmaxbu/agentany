...
async function setup(streamCtor: (deps: RunDeps, log: { calls: number; appends: string[][] }) => ConfiguredRunPiStream = makeJudgeStream) {
  ...
  const conv = (cc: string, userId: string) => store.chat.createConversation({ id: cc, workspaceId: "ws_company", userId });
...
describe("入站 · 无 pending → 零副作用丢弃（决策 2/5）", () => {
  test("未绑定 IM 身份 → discarded，不落库不起轮", async () => {
    ...
    expect(ctx.store.chat.listMessages("c1")).toHaveLength(0); // 不落库
  ...
  test("纯 approval/task 卡命中（决策 5）→ discarded + 回发「去 Web/App 点卡」提示，不进文本回流", async () => {
    ...
    ctx.store.hitl.createQuestion({ conversationId: "c1", kind: "approval", workflowId: "brand-research", input: {}, prompt: "需审批", options: ["批准", "拒绝"] });
    ctx.store.hitl.createQuestion({ conversationId: "c1", runId: null, kind: "task", prompt: "建任务卡", options: ["确认创建", "取消"], input: { displayName: "t", cron: "0 3 * * *", prompt: "p" } });
    const frames: any[] = []; ctx.eventBus.subscribe("c1", (f) => frames.push(f));
    ...
    expect(ctx.store.chat.listMessages("c1")).toHaveLength(0); // 不落库
    expect(frames).toHaveLength(0); // 不推帧
    expect(ctx.store.hitl.listQuestions("c1", { includeAnswered: true }).filter((q) => q.status === "pending")).toHaveLength(2); // 卡未动
  });
  ...
  test("队列忙 → busy（不落库不判答）", async () => {
    ...
    ctx.conv("c1", m1.id);
    ctx.deps.imStore!.bind("tg-1", "telegram", m1.id);
    ctx.store.hitl.createQuestion({ conversationId: "c1", runId: "r-p", prompt: "选？", options: ["A"], resumeSchema: { _t: "enum", vals: ["A"] } });
    ...
    expect(ctx.store.chat.listMessages("c1")).toHaveLength(0);
...
describe("入站 · 有 pending ask 卡 → 内联 turn 判答收口", () => {
  test("run 绑定卡：定位会话 + 消息落库 + 注入含 [待处理提问]/[挂起工作流] + 卡 answered + hitl_answered 广播 + 回发文本", async () => {
    ...
    ctx.store.runs.createRun({ runId, workflowId: "synthetic-3step", workspaceId: "ws_company", conversationId: "c2", input: {} });
    ctx.store.runs.updateRunStatus(runId, "suspended");
    ctx.store.runs.appendLog(runId, { stepId: "review", status: "suspended", suspendPayload: { options: ["accept", "redirect"] },
      resumeSchema: { _t: "object", shape: { decision: { _t: "enum", vals: ["accept", "redirect"] }, focus: { _t: "optional", inner: { _t: "string" } } } } });
    const qid = ctx.store.hitl.createQuestion({ conversationId: "c2", runId, prompt: "选哪个？", options: ["accept", "redirect"],
      resumeSchema: { _t: "object", shape: { decision: { _t: "enum", vals: ["accept", "redirect"] }, focus: { _t: "optional", inner: { _t: "string" } } } },
    ...
    expect(ctx.store.chat.listMessages("c2").some((m) => m.role === "user" && m.content === "选 A方案")).toBe(true); // 文本进会话历史（与 Web 打字同构）
    ...
    expect(ctx.store.hitl.getQuestion(qid)!.status).toBe("answered"); // 卡 CAS 收口
  ...
  test("自主卡：注入引导 answer_question → 落卡 + 广播（决策 10 修订同款）", async () => {
    ...
    ctx.conv("c1", m1.id);
    ctx.deps.imStore!.bind("tg-1", "telegram", m1.id);
    const qid = ctx.store.hitl.createQuestion({ conversationId: "c1", runId: null, prompt: "澄清：预算区间？", options: ["<10w", ">50w"] });
    const frames: any[] = []; ctx.eventBus.subscribe("c1", (f) => frames.push(f));
...
describe("T6 决策：扫全部 ask 卡三分支（kind 拦截仅 0 ask 时）", () => {
  test("0 张 ask（纯 approval/task）→ 最新 approval/task → 只提示去 Web/App（决策 5 保留）", async () => {
    ...
    ctx.conv("c1", m1.id);
    ctx.deps.imStore!.bind("tg-1", "telegram", m1.id);
    ctx.store.hitl.createQuestion({ conversationId: "c1", kind: "approval", workflowId: "brand-research", input: {}, prompt: "审批", options: ["批准", "拒绝"] });
    ctx.store.hitl.createQuestion({ conversationId: "c1", runId: null, kind: "task", prompt: "建任务", options: ["确认", "取消"], input: {} }); // 后建 → 最新 pending
  ...
  test("1 张 ask + 新 approval 并存 → T6 口径：有 ask 即可答 ask（approval 是按钮通道不拦文本）", async () => {
    ...
    ctx.conv("c1", m1.id);
    ctx.deps.imStore!.bind("tg-1", "telegram", m1.id);
    const askQid = ctx.store.hitl.createQuestion({ conversationId: "c1", runId: null, prompt: "旧澄清", options: ["旧A"] });
    ctx.store.hitl.createQuestion({ conversationId: "c1", kind: "approval", workflowId: "brand-research", input: {}, prompt: "新审批", options: ["批准", "拒绝"] });
    ...
    expect(ctx.store.hitl.getQuestion(askQid)!.status).toBe("answered");
  });
});
...
describe("双端竞态：Web 点卡 vs IM 内联 turn → CAS 单次执行", () => {
  async function raceSetup() {
    ...
    ctx.deps.imStore!.bind("tg-1", "telegram", m1.id);
    const runId = "r_race";
    ctx.store.runs.createRun({ runId, workflowId: "synthetic-3step", workspaceId: "ws_company", conversationId: "c1", input: {} });
    ctx.store.runs.updateRunStatus(runId, "suspended");
    ctx.store.runs.appendLog(runId, { stepId: "review", status: "suspended", suspendPayload: { options: ["accept", "redirect"] },
      resumeSchema: { _t: "object", shape: { decision: { _t: "enum", vals: ["accept", "redirect"] }, focus: { _t: "optional", inner: { _t: "string" } } } } });
    const qid = ctx.store.hitl.createQuestion({ conversationId: "c1", runId, prompt: "选哪个？", options: ["accept", "redirect"],
      resumeSchema: { _t: "object", shape: { decision: { _t: "enum", vals: ["accept", "redirect"] }, focus: { _t: "optional", inner: { _t: "string" } } } },
      values: [{ label: "accept", value: { decision: "accept" } }, { label: "redirect", value: { decision: "redirect" } }] });
    const frames: any[] = []; ctx.eventBus.subscribe("c1", (f) => frames.push(f));
  ...
  test("并行：Web dispatch 与 IM 内联几乎同时 → 卡 answered 一次、hitl_answered 一条、run 不收两发", async () => {
    ...
    expect(ctx.store.hitl.getQuestion(qid)!.status).toBe("answered"); // 卡只 answered 一次
    expect(frames.filter((f) => f.type === "hitl_answered" && f.questionId === qid)).toHaveLength(1);
    await delayUntil(() => ctx.store.runs.getRun(runId)!.status === "completed"); // run 只续跑一次
  });
  ...
  test("顺序 · dispatch 先胜 → IM 后至按无 pending 丢弃（幂等 ack）", async () => {
    ...
    await delayUntil(() => ctx.store.runs.getRun(runId)!.status === "completed"); // 单次续跑
  });
  ...
  test("dispatch 直调：已答后 dispatchCardAnswer 幂等 no-op（skipTurn 不另起轮）", async () => {
    ...
    await ctx.store.hitl.markPendingAnsweredByRun(runId, { decision: "accept" }); // 模拟 IM 已先收口
