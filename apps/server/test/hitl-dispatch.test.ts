...
async function setup() {
  ...
  const deps: RunDeps = {
    ...
    eventBus,
    runRegistry: new RunRegistry({ store, eventBus, runPiFactory: stubFactory }),
  };
  store.createConversation({ id: "c1", workspaceId: "ws_company", userId: m1.id });
  store.createConversation({ id: "c2", workspaceId: "ws_company", userId: m1.id });
...
describe("dispatch · task 卡（消息绑定确认）", () => {
  test("inReplyTo 指向 task pending 卡，content=确认创建 → 服务端直建 + 产出会话 + 卡 answered", async () => {
    ...
    expect(ctx.store.getConversation(mine[0].outputConversationId!)!.title).toBe("新闻汇总");
  ...
  test("他人消息绑我的卡 → 不生效（卡仍 pending，任务不建）", async () => {
    ...
    await new Promise((res) => setTimeout(res, 20));
    expect(ctx.store.getQuestion(qid)!.status).toBe("pending");
    expect(ctx.deps.taskStore!.listTasks({ creatorId: ctx.m1.id })).toHaveLength(0);
  });
});
...
describe("dispatch · approval 卡（收编 #18）", () => {
  test("content=批准（options[0]）→ decide approve + createRun（approved:true 跳 policy）+ backfill", async () => {
    ...
    expect(ctx.store.getRun(q.runId!)!.workflowId).toBe("brand-research");
  });
