...
describe("GET /conversations 列表", () => {
  ...
  const mk = (id: string, workspaceId: string, userId: string, title = "") =>
    deps.chatStore.createConversation({ id, workspaceId, userId, title });
  ...
  test("按 userId 过滤 + workspaceId 过滤 + 倒序（updatedAt）", async () => {
    ...
    const a1 = deps.chatStore.createConversation({ id: "c1", workspaceId: "ws_company", userId: "u1", title: "a" });
    void a1;
    deps.chatStore.createConversation({ id: "c2", workspaceId: "ws_company", userId: "u1", title: "b" });
    deps.chatStore.createConversation({ id: "c3", workspaceId: "ws_acme", userId: "u1", title: "c" });
    deps.chatStore.createConversation({ id: "c4", workspaceId: "ws_company", userId: "u2", title: "d" });

    const mine = deps.chatStore.listConversations("u1");
    expect(mine.map((c) => c.id).sort()).toEqual(["c1", "c2", "c3"]);
    const inWs = deps.chatStore.listConversations("u1", "ws_company");
    expect(inWs.map((c) => c.id).sort()).toEqual(["c1", "c2"]);
    expect(deps.chatStore.listConversations("u2").map((c) => c.id)).toEqual(["c4"]);
  });

  test("updatedAt 倒序：touch 后排到最前（同秒时间戳也真验证）", () => {
    deps.chatStore.createConversation({ id: "c1", workspaceId: "ws_company", userId: "u1" });
    deps.chatStore.createConversation({ id: "c2", workspaceId: "ws_company", userId: "u1" });
    // c1 被发消息 → touch → updatedAt 更新 → 排最前（真倒序验证：不依赖两条创建时间戳有差异，
    // 同毫秒并列时顺序未定义，touch 引入确定差异）
    deps.chatStore.touchConversation("c1");
    expect(deps.chatStore.listConversations("u1").map((c) => c.id)).toEqual(["c1", "c2"]);
    // touch 不存在的会话：no-op 不炸
    deps.chatStore.touchConversation("c_nope");
    expect(deps.chatStore.listConversations("u1").map((c) => c.id)).toEqual(["c1", "c2"]);
  });
